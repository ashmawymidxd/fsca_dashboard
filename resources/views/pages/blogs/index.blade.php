                            ...
                            <tbody id="sortable">
                                @foreach ($blogs as $blog)
                                    <tr data-id="{{ $blog->id }}">
                                        ...
                                        <td>
                                            <img src="{{ asset($blog->cover_image) }}" alt="{{ $blog->title_en }}" class="img-thumbnail" style="max-width: 100px;">
                                        </td>
                                        <td>{{ $blog->title_en }}</td>
                                        <td>{{ Str::limit($blog->description_en, 50) }}</td>
                                        <td>{{ ucfirst($blog->image_direction) }}</td>
                                        <td>{{ $blog->created_at->format('d/m/Y') }}</td>
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('blogs.show', $blog) }}" class="btn btn-sm btn-info mr-2"><i class="fas fa-eye"></i></a>
                                                <a href="{{ route('blogs.edit', $blog) }}" class="btn btn-sm btn-primary mr-2"><i class="fas fa-edit"></i></a>
...
$(document).ready(function() {
    const sortable = new Sortable(document.getElementById('sortable'), {
        ...
        onEnd: function() {
            ...
            $.post("{{ route('blogs.reorder') }}", {blogs: order, _token: "{{ csrf_token() }}"}, function(res){
                if(res.success) toastr.success('Order updated successfully');
            }).fail(function(){ toastr.error('Error updating order'); location.reload(); });
        }
    });
