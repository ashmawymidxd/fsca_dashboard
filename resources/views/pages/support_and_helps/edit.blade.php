...
@section('content')
    @include('users.partials.header', [
        'title' => __('Edit Support & Help Item'),
        'description' => __('Update the support and help details'),
        'class' => 'col-lg-12',
    ])
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ __('Edit Support & Help') }}</h3>
                            </div>
                    ...
                    <div class="card-body bg-white">
                        <form method="post" action="{{ route('support-and-helps.update', $supportAndHelp) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group">
                                    ...
                                    <div>
                                        <img src="{{ asset( $supportAndHelp->cover_image) }}" width="150" class="rounded mb-2">
                                    </div>
                                ...
                                <div class="form-group">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" name="is_active" class="custom-control-input" id="is_active"
                                            {{ $supportAndHelp->is_active ? 'checked' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title_en" id="title_en" class="form-control"
                                        value="{{ $supportAndHelp->translation('en')->title ?? '' }}" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-control-label" for="description_en">{{ __('Description') }}</label>
                                    <textarea name="description_en" id="description_en" class="form-control" rows="5" required>{{ $supportAndHelp->translation('en')->description ?? '' }}</textarea>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title_ar" id="title_ar" class="form-control" dir="rtl"
                                        value="{{ $supportAndHelp->translation('ar')->title ?? '' }}" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-control-label" for="description_ar">{{ __('Description') }}</label>
                                    <textarea name="description_ar" id="description_ar" class="form-control" rows="5" dir="rtl" required>{{ $supportAndHelp->translation('ar')->description ?? '' }}</textarea>
                                </div>
        ...
        document.getElementById('cover_image').addEventListener('change', function(e) {
            if (e.target.files.length > 0) {
                ...
                reader.onload = function(event) {
                    document.querySelector('.rounded.mb-2').src = event.target.result;
                };
