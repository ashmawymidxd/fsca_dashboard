...
@section('content')
    @include('users.partials.header', [
        'title' => __('Edit Sustainability'),
        'description' => __('Update the sustainability details'),
        'class' => 'col-lg-12',
    ])
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ __('Edit Sustainability') }}</h3>
                            </div>
                    ...
                    <div class="card-body bg-white">
                        <form method="post" action="{{ route('sustainabilities.update', $sustainability) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group">
                                    ...
                                    <div>
                                        <img src="{{ asset($sustainability->cover_image) }}" width="150" class="rounded mb-2" id="current_image_preview">
                                    </div>
                                ...
                                <div class="form-group">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" name="is_active" class="custom-control-input" id="is_active"
                                            {{ $sustainability->is_active ? 'checked' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title_en" id="title_en" class="form-control"
                                        value="{{ $sustainability->translation('en')->title ?? '' }}" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-control-label" for="description_en">{{ __('Description') }}</label>
                                    <textarea name="description_en" id="description_en" class="form-control" rows="3" required>{{ $sustainability->translation('en')->description ?? '' }}</textarea>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title_ar" id="title_ar" class="form-control" dir="rtl"
                                        value="{{ $sustainability->translation('ar')->title ?? '' }}" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-control-label" for="description_ar">{{ __('Description') }}</label>
                                    <textarea name="description_ar" id="description_ar" class="form-control" rows="3" dir="rtl" required>{{ $sustainability->translation('ar')->description ?? '' }}</textarea>
                                </div>

                                <div class="text-center">
                                    <button type="submit" class="btn btn-success mt-4">{{ __('Update Sustainability') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
